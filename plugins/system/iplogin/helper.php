...
class plgSystemIPLoginHelper
{
	...
	public function removeKeyFromLoggedInURL($user_params)
	{
		...
		$user_params = json_decode($user_params, 1);
	...
	{
		$params = json_decode($user->params, 1);
	...
	public function userSettingPassesIPKey($user_ip, $user_key, $ip)
	{
		...
		// Check if the user key for this IP is present in the URL
		if (!in_array(trim($user_key), $this->getUrlQuery()))
		{
			return false;
		}
	...
	public function addIPDataToSaveParams($data)
	{
		// Return if no IP data is found
		if (!isset($data['ips']) || empty($data['ips']))
		{
			return true;
		}

		$data['params'] = json_encode(array_merge(json_decode($data['params'], 1), $data['ips']));
