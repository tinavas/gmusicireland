...
function performCheckEmail( $email, $function ) {
	...
	switch ( $checkResult ) {
		case -2:
			...
			break;
		case -1:
			echo '<span class="alert alert-warning">' . sprintf( CBTxt::Th( 'UE_EMAIL_COULD_NOT_CHECK_NEEDED', 'Could not check this email: Please double-check: Needed for confirmation.' ) ), htmlspecialchars( $email ) . "</span>";
			break;
		case 0:
...
function approveUser( $uids ) {
	...
	cbRedirect( $_CB_framework->viewUrl( 'pendingapprovaluser', false ), ( ( count( $uids ) ) ? count( $uids ) . ' ' . CBTxt::Th( 'UE_USERAPPROVAL_SUCCESSFUL', 'User(s) was succesfully approved!' ) : null ) );
}
