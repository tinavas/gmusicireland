...
class CBRegisterFormView_html_default extends cbRegistrationView
{
	...
	private function _renderRegistrationHead( )
	{
		...
		$return				.=	'<div class="cb_template cb_template_' . selectTemplate( 'dir' ) . ' cbRegistration ' . ( $tabbed ? 'cbRegistrationTabbed' : 'cbRegistrationFlat' ) . ( $canvasImg ? ' cbRegistrationCanvas' : null ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : null ) . '">';
	...
	/**
	 * Renders by ECHO the registration view (table mode)
	 *
	 * @return void
	 */
...
class CBListView_html_default extends cbListView
{
	...
	protected function _renderHead( )
	{
		...
		if ( ( ( count( $this->lists ) > 0 ) && $this->allowListSelector ) || $this->searchTabContent ) {
