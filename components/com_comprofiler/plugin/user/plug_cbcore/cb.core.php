...
class CBfield_date extends cbFieldHandler {
			...
			case 'rss':
				if ( ( $value != null ) && ( $value != '' ) && ( $value != '0000-00-00 00:00:00' ) && ( $value != '0000-00-00' ) ) {
					switch ( (int) $field->params->get( 'field_display_by', 0 ) ) {
						case 1: // Age
							$currentYear	=	(int) cbFormatDate( 'now', true, true, 'Y' );
							$dateOfBirth	=	(int) cbFormatDate( $value, true, true, 'Y' );
							$age			=	(int) ( $currentYear - $dateOfBirth );
						...
						case 2: // Timeago, with Ago
							$return			=	$this->formatFieldValueLayout( cbFormatDate( $value, true, 'timeago' ), $reason, $field, $user, false );
							break;
						case 6: // Timeago, without Ago
							$return			=	$this->formatFieldValueLayout( cbFormatDate( $value, true, 'exacttimeago' ), $reason, $field, $user, false );
							break;
						case 3: // Birthdate
							$return			=	$this->formatFieldValueLayout( htmlspecialchars( cbFormatDate( $value, true, true, 'F d' ) ), $reason, $field, $user );
							break;
						case 4: // Date
							$return			=	$this->formatFieldValueLayout( htmlspecialchars( cbFormatDate( $value, true, false ) ), $reason, $field, $user );
							break;
						case 5: // Custom
							$dateFormat		=	$field->params->get( 'custom_date_format', 'Y-m-d' );
							$timeFormat		=	$field->params->get( 'custom_time_format', 'H:i:s' );

							$return			=	$this->formatFieldValueLayout( htmlspecialchars( cbFormatDate( $value, true, ( $field->get( 'type' ) == 'datetime' ? true : false ), $dateFormat, $timeFormat ) ), $reason, $field, $user );
							break;
						default: // Date/Datetime
							$return			=	$this->formatFieldValueLayout( htmlspecialchars( cbFormatDate( $value, true, ( $field->get( 'type' ) == 'datetime' ? true : false ) ) ), $reason, $field, $user );
							break;
					}
				} else {
			...
			case 'htmledit':
				...
				if ( $reason == 'search' ) {
					...
					if ( $searchBy == 1 ) {
						...
						$minHtml			=	$calendars->cbAddCalendar( $minNam, CBTxt::Th( 'UE_SEARCH_FROM', 'Between' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $minVal, false, ( $field->get( 'type' ) == 'datetime' ? true : false ), $yMin, $yMax, $tooltip );
						$maxHtml			=	$calendars->cbAddCalendar( $maxNam, CBTxt::Th( 'UE_SEARCH_TO', 'and' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $maxVal, false, ( $field->get( 'type' ) == 'datetime' ? true : false ), $yMin, $yMax, $tooltip );
					}
					...
					$return					=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $field->get( 'name' ), $this->getFieldTitle( $field, $user, 'text', $reason ), $this->_isRequired( $field, $user, $reason ), $value, $this->_isReadOnly( $field, $user, $reason ), ( $field->get( 'type' ) == 'datetime' ? true : false ), $yMin, $yMax, $tooltip ), $reason, $field, $user )
	...
	function _yearSetting( $setParam, $outputMode ) {
		...
		if ( $outputMode == 1 ) {
			if ( $offset === null ) {
				$offset			=	( $fullYear - (int) cbFormatDate( 'now', true, true, 'Y' ) );
			}

			return -$offset;
		} else {
			if ( $offset !== null ) {
				$fullYear		=	( (int) cbFormatDate( 'now', true, true, 'Y' ) + $offset );
			}
								...
								switch ( $checkResult ) {
									...
									case -1:
										...
										if ( $emailConfirmation ) {
											$message	=	CBTxt::Th( 'UE_EMAIL_COULD_NOT_CHECK_NEEDED', 'Could not check this email: Please double-check: Needed for confirmation.', array( '[email]' =>  htmlspecialchars( $email ) ) );
										} else {
											$message	=	CBTxt::Th( 'UE_EMAIL_COULD_NOT_CHECK', 'Could not check this email: Please double-check.', array( '[email]' =>  htmlspecialchars( $email ) ) );
										}
										break;
									case 0:
										$valid			=	false;
...
class CBfield_image extends cbFieldHandler {
		...
		switch ( $columnName ) {
			case 'upload':
				if ( ! $field->params->get( 'image_allow_uploads', 1 ) ) {
					...
					$upload_mime_limit		=	cbGetMimeFromExt( $upload_ext_limit );
					...
					if ( ! $uploaded_ext ) {
						...
						if ( ( ! in_array( $uploaded_ext, $upload_ext_limit ) ) || ( ! in_array( $value['type'], $upload_mime_limit ) ) ) {
							$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'Please upload only %s' ), implode( ', ', $upload_ext_limit ) ) );
							return false;
						}
					}
					...
					if ( ( $uploaded_size / 1024 ) > $upload_size_limit_max ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'The image file size exceeds the maximum of %s KB' ), $upload_size_limit_max ) );
						return false;
					}
				}
				break;
	...
	function _htmlEditForm( &$field, &$user, $reason, $displayFieldIcons = true ) {
		...
		$restrictions							=	CBTxt::Th( 'YOUR_IMAGE_MUST_BE_OF_EXT_TYPE_AND_WILL_BE_RESIZED', 'Your image must be of [ext] type and will be resized if needed to a maximum dimension of [width] pixels width x [height] height automatically, but your image file should not exceed [max] KB.',
															  array( '[width]' => $uploadWidthLimit, '[height]' => $uploadHeightLimit, '[max]' => $uploadSizeLimitMax, '[ext]' => implode( ', ', $uploadExtLimit ) )
															);
		...
		if ( count( $choices ) > 1 ) {
			...
			$return								.=		'<input type="hidden" name="' . htmlspecialchars( $fieldName ) . '__choice" value="' . $choices[0]->value . '" />';
		}
		...
		if ( $field->params->get( 'image_allow_uploads', 1 ) ) {
			...
			$validationAttributes[]				=	cbValidator::getRuleHtmlAttributes( 'accept', implode( ',', cbGetMimeFromExt( $uploadExtLimit ) ) );
...
class CBfield_delimiter extends cbFieldHandler {
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types ) {
		...
		$return			=	$this->_formatFieldOutput( $field->name, $value, $output, false )
						.	( $output == 'htmledit' ? $this->_fieldIconsHtml( $field, $user, $output, $reason, null, null, $value, null, null, false, false ) : null );
	...
	public function getFieldRow( &$field, &$user, $output, $formatting, $reason, $list_compare_types ) {
		...
		if ( $ui == 2 ) {
			...
			if ( $canBlockUser ) {
				...
				$lists['block']					=	moscomprofilerHTML::yesnoSelectList( 'block', 'class="form-control"', (int) $user->block );
				...
				$lists['confirmed']				=	moscomprofilerHTML::yesnoSelectList( 'confirmed', 'class="form-control"', (int) $user->confirmed, CBTxt::T( 'Confirmed' ), CBTxt::T( 'Pending' ) );
				// build the html select list
...
class CBfield_file extends cbFieldHandler {
	...
	function _htmlEditForm( &$field, &$user, $reason, $displayFieldIcons = true ) {
		...
		$restrictions						=	CBTxt::Th( 'YOUR_FILE_MUST_BE_OF_EXT_TYPE_AND_SHOULD_EXCEED_BUT_NOT_EXCEED', 'Your file must be of [ext] type and should exceed [min] KB, but not [max] KB',
														  array( '[max]' => $uploadSizeLimitMax, '[min]' => $uploadSizeLimitMin, '[ext]' => implode( ', ', $uploadExtLimit ) )
														);
		...
		$return								.=		$this->_fieldIconsHtml( $field, $user, 'htmledit', $reason, 'select', '', null, '', array(), $displayFieldIcons, $required )
											.	'</div>'
											.	'<div id="cbfile_upload_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
											.		'<p>' . $restrictions . '</p>'
											.		'<div>'
...
class CBfield_video extends CBfield_text {
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types ) {
		...
		switch ( $output ) {
			...
			case 'rss':
				if ( $value ) {
					$return			=	$this->getEmbed( $field, $user, $value, ( $reason != 'profile' ) );
				}
	 ...
	 * @return null|string
	 */
	public function getEmbed( $field, $user, $value, $thumb = false ) {
		global $_CB_framework;
		...
		if ( $urlDomain ) {
			...
			if ( $thumb ) {
				$height				=	(int) $field->params->get( 'video_thumbheight', 250 );
			} else {
			...
			if ( $mimeType && in_array( $mimeType, $this->allowedMimeTypes() ) ) {
				$embed				=	'<video width="' . (int) $width . '" height="' . (int) $height . '" src="' . htmlspecialchars( $value ) . '" type="' . htmlspecialchars( $mimeType ) . '" preload="auto" class="cbVideoField"></audio>';
			}
			...
			if ( $embed ) {
				...
				if ( ! $JS_loaded++ ) {
					$_CB_framework->outputCbJQuery( "$( '.cbVideoField' ).mediaelementplayer();", 'media' );
				}
			}
		}
	...
	function _htmlEditForm( &$field, &$user, $output, $reason, $displayFieldIcons = true ) {
		...
		$uploadMimeLimit					=	$this->allowedMimeTypes( true );

		$restrictions						=	CBTxt::Th( 'YOUR_VIDEO_FILE_MUST_BE_OF_EXT_TYPE_AND_SHOULD_EXCEED_BUT_NOT_EXCEED', 'Your video file must be of [ext] type and should exceed [min] KB, but not [max] KB',
														  array( '[max]' => $uploadSizeLimitMax, '[min]' => $uploadSizeLimitMin, '[ext]' => implode( ', ', $uploadMimeLimit ) )
														);
		...
		if ( ( $reason != 'register' ) && ( $user->id != 0 ) && $existingFile ) {
			$return							.=	'<div class="form-group cb_form_line clearfix">' . $this->getEmbed( $field, $user, $value, true ) . '</div>';
		}
		...
		$return								.=		$this->_fieldIconsHtml( $field, $user, 'htmledit', $reason, 'select', '', null, '', array(), $displayFieldIcons, $required )
											.	'</div>'
											.	'<div id="cbvideofile_upload_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
											.		'<p>' . $restrictions . '</p>'
											.		'<div>'
											.			CBTxt::T( 'Select video file' ) . ' <input type="file" name="' . htmlspecialchars( $fieldName ) . '__file" value="" class="form-control' . ( $required == 1 ? ' required' : null ) . '"' . cbValidator::getRuleHtmlAttributes( 'accept', implode( ',', $uploadMimeLimit ) ) . ' />'
											.		'</div>'
											.		'<p>';
		...
		switch ( $choice ) {
			case 'upload':
				...
				if ( $this->validate( $field, $user, $choice, $value, $postdata, $reason ) ) {
					...
					$uploadedName	=	preg_replace( '/[^-a-zA-Z0-9_]/', '', pathinfo( $value['name'], PATHINFO_FILENAME ) );
					$uploadedExt	=	strtolower( preg_replace( '/[^-a-zA-Z0-9_]/', '', pathinfo( $value['name'], PATHINFO_EXTENSION ) ) );
					$newFileName	=	uniqid( $uploadedName . '_' ). '.' . $uploadedExt;
		...
		switch ( $choice ) {
			case 'upload':
				if ( ! $field->params->get( 'video_allow_uploads', 1 ) ) {
					...
					if ( ( ! $uploadedExt ) || ( ! in_array( cbGetMimeFromExt( $uploadedExt ), $uploadMimeLimit ) ) || ( ! isset( $value['type'] ) ) || ( ! in_array( $value['type'], $uploadMimeLimit ) ) ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'Please upload only %s' ), implode( ', ', $uploadMimeLimit ) ) );
						return false;
					}
					...
					if ( ( $uploadedSize / 1024 ) > $uploadSizeLimitMax ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'The video file size exceeds the maximum of %s KB' ), $uploadSizeLimitMax ) );
						return false;
					}

					if ( ( $uploadedSize / 1024 ) < $uploadSizeLimitMin ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'The video file is too small, the minimum is %s KB' ), $uploadSizeLimitMin ) );
						return false;
					}
				}
				break;
			case 'link':
				...
				if ( $validated && ( $value !== '' ) && ( $value !== null ) ) {
					$linkMimeLimit			=	$this->allowedMimeTypes();
					$mimeType				=	$this->getMimeType( $value );

					if ( ( ! $mimeType ) || ( ! in_array( $mimeType, $linkMimeLimit ) ) ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'Please link only %s' ), implode( ', ', $linkMimeLimit ) ) );
	...
	function getMimeType( $value ) {
		$urlDomain					=	preg_replace( '/^(?:(?:\w+\.)*)?(\w+)\..+$/', '\1', parse_url( $value, PHP_URL_HOST ) );
		$mimeType					=	cbGetMimeFromExt( strtolower( pathinfo( $value, PATHINFO_EXTENSION ) ) );

		if ( ( ! in_array( $mimeType, $this->allowedMimeTypes() ) ) && $urlDomain ) {
			if ( in_array( $urlDomain, array( 'youtube', 'youtu' ) ) ) {
				$mimeType			=	'video/youtube';
			} elseif ( $urlDomain == 'vimeo' ) {
				...
				$contentType		=	cbGetParam( $headers, 'content-type' );

				if ( $contentType ) {
					if ( is_array( $contentType ) ) {
						$mimeType	=	stripslashes( array_pop( $contentType ) );
					} else {
						$mimeType	=	stripslashes( $contentType );
					}
				}
			}
		}
	 ...
	 * @return array
	 */
	function allowedMimeTypes( $uploadOnly = false ) {
		$allowed		=	array( 'video/mpeg', 'video/mp4', 'video/ogg', 'video/quicktime', 'video/webm', 'video/x-ms-wmv', 'video/x-flv' );

		if ( ! $uploadOnly ) {
			$allowed	=	array_merge( $allowed, array( 'video/youtube', 'video/vimeo', 'video/rtmp' ) );
		}
...
class CBfield_audio extends CBfield_text {
	...
	public function getEmbed( $field, $user, $value, $thumb = false ) {
		...
		if ( $urlDomain ) {
			...
			if ( $thumb ) {
				$height				=	(int) $field->params->get( 'audio_thumbheight', 30 );
			} else {
				$height				=	(int) $field->params->get( 'audio_height', 30 );
			}
			...
			if ( $mimeType && in_array( $mimeType, $this->allowedMimeTypes() ) ) {
				$embed				=	'<audio width="' . (int) $width . '" height="' . (int) $height . '" src="' . htmlspecialchars( $value ) . '" type="' . htmlspecialchars( $mimeType ) . '" preload="auto" class="cbAudioField"></audio>';
			}
			...
			if ( $embed ) {
				...
				if ( ! $JS_loaded++ ) {
					$_CB_framework->outputCbJQuery( "$( '.cbAudioField' ).mediaelementplayer();", 'media' );
				}
			}
		}
	...
	function _htmlEditForm( &$field, &$user, $output, $reason, $displayFieldIcons = true ) {
		...
		$restrictions						=	CBTxt::Th( 'YOUR_AUDIO_FILE_MUST_BE_OF_EXT_TYPE_AND_SHOULD_EXCEED_BUT_NOT_EXCEED', 'Your audio file must be of [ext] type and should exceed [min] KB, but not [max] KB',
														  array( '[max]' => $uploadSizeLimitMax, '[min]' => $uploadSizeLimitMin, '[ext]' => implode( ', ', $uploadMimeLimit ) )
														);
		...
		$return								.=		$this->_fieldIconsHtml( $field, $user, 'htmledit', $reason, 'select', '', null, '', array(), $displayFieldIcons, $required )
											.	'</div>'
											.	'<div id="cbaudiofile_upload_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
											.		'<p>' . $restrictions . '</p>'
											.		'<div>'
											.			CBTxt::T( 'Select audio file' ) . ' <input type="file" name="' . htmlspecialchars( $fieldName ) . '__file" value="" class="form-control' . ( $required == 1 ? ' required' : null ) . '"' . cbValidator::getRuleHtmlAttributes( 'accept', implode( ',', $uploadMimeLimit ) ) . ' />'
											.		'</div>'
											.		'<p>';
		...
		switch ( $choice ) {
			case 'upload':
				if ( ! $field->params->get( 'audio_allow_uploads', 1 ) ) {
					...
					if ( ( $uploadedSize / 1024 ) > $uploadSizeLimitMax ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'The audio file size exceeds the maximum of %s KB' ), $uploadSizeLimitMax ) );
						return false;
					}

					if ( ( $uploadedSize / 1024 ) < $uploadSizeLimitMin ) {
						$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T( 'The audio file is too small, the minimum is %s KB' ), $uploadSizeLimitMin ) );
						return false;
					}
				}
				break;
		...
		if ( ! isset( $cache[$cacheId] ) ) {
			...
			switch ( $ratingAccess ) {
				...
				case 7:
					...
					break;
				case 6:
					if ( ( $userId != $myId ) || ( ( $userId == $myId ) && $includeSelf ) ) {
		...
		if ( ! $value ) {
			...
			if ( $ratingId ) {
				$query		=	'UPDATE ' . $_CB_database->NameQuote( '#__comprofiler_ratings' )
							...
							.	', ' . $_CB_database->NameQuote( 'date' ) . " = NOW()"
...
class CBfield_points extends CBfield_integer
{
	...
	private function getPointsHTML( &$field, &$user, $reason, $ajax = false ) {
		...
		if ( $canIncrement ) {
			...
			$plusIcon					=	'<span class="' . ( $plusCSS ? htmlspecialchars( $plusCSS ) : 'fa fa-plus-circle' ) . '"></span>';
			$minusIcon					=	'<span class="' . ( $minusCSS ? htmlspecialchars( $minusCSS ) : 'fa fa-minus-circle' ) . '"></span>';
		...
		if ( ( $output == 'htmledit' ) && ( $reason != 'search' ) ) {
			...
			if ( $termsDisplay == 'iframe' ) {
				if ( $termsOutput == 'url' ) {
					...
					$return				.=	'<div class="cbTermsFrameText" style="height:' . $termsHeight . 'px;width:' . ( $termsWidth ? $termsWidth . 'px' : '100%' ) . ';overflow:auto;">' . $termsText . '</div>';
				}
			}
			...
			if ( $termsDisplay != 'iframe' ) {
				...
				if ( $termsDisplay == 'modal' ) {
					...
					if ( $termsOutput == 'url' ) {
						...
						$tooltip		=	'<div class="cbTermsModalText" style="height:' . $termsHeight . 'px;width:' . $termsWidth . 'px;overflow:auto;">' . $termsText . '</div>';
					}
