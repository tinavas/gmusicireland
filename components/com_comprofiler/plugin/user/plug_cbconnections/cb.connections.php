			...
			if ( is_array( $conGroups ) && ( count( $conGroups ) > 2 ) ) {
				...
				if ( isset( $connections[$user->id] ) ) {
					$return					.=	'<div class="cbConnectionPaths alert alert-info">'
											.		CBTxt::Th( 'CONNECTIONS_YOUR_CONNECTION_PATH_TO_USER_OF_DEGREE_IS', 'Your connection path to [user] of [degrees] degress is ', array( '[user]' => $myAvatar, '[degrees]' => $cbCon->getDegreeOfSep() ) );
