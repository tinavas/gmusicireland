...
class cbblogsBlogTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		if ( ! $this->get( 'id' ) ) {
			...
			if ( $this->get( 'category' ) == '' ) {
				$this->set( 'category', $plugin->params->get( 'blog_int_category_default', 'General' ) );
			}

			if ( ! $this->get( 'published' ) ) {
				$this->set( 'published', (int) $plugin->params->get( 'blog_approval', 0 ) );
			}

			if ( ! $this->get( 'access' ) ) {
				$this->set( 'access', (int) $plugin->params->get( 'blog_access_default', 1 ) );
			}
		}
	...
	public function bind( $array, $ignore = '', $prefix = null )
	{
		...
		if ( $bind ) {
			...
			$this->set( 'category', ( ( $plugin->params->get( 'blog_category_config', 1 ) || $isModerator ) ? Get::get( $array, 'category', $this->get( 'category' ), GetterInterface::STRING ) : $this->get( 'category', $plugin->params->get( 'blog_int_category_default', 'General' ) ) ) );
			$this->set( 'published', (int) ( ( ( ! $plugin->params->get( 'blog_approval', 0 ) ) || $isModerator ) ? Get::get( $array, 'published', $this->get( 'published' ), GetterInterface::INT ) : $this->get( 'published', ( $isModerator || ( ! $plugin->params->get( 'blog_approval', 0 ) ) ? 1 : 0 ) ) ) );
			$this->set( 'access', (int) ( ( $plugin->params->get( 'blog_access_config', 1 ) || $isModerator ) ? Get::get( $array, 'access', $this->get( 'access' ), GetterInterface::INT ) : $this->get( 'access', $plugin->params->get( 'blog_access_default', 1 ) ) ) );
