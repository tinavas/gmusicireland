...
class cbblogsBlogTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		if ( ! $article->get( 'id' ) ) {
			...
			if ( ! $this->get( 'catid' ) ) {
				$this->set( 'catid', (int) $plugin->params->get( 'blog_j_category_default', null ) );
			}

			if ( ! $this->get( 'state' ) ) {
				$this->set( 'state', (int) $plugin->params->get( 'blog_approval', 0 ) );
			}
	...
	public function bind( $array, $ignore = '', $prefix = null )
	{
		...
		if ( $bind ) {
			...
			$this->set( 'catid', (int) ( ( $plugin->params->get( 'blog_category_config', 1 ) || $isModerator ) ? Get::get( $array, 'category', $this->get( 'catid' ), GetterInterface::INT ) : $this->get( 'catid', $plugin->params->get( 'blog_j_category_default', null ) ) ) );
			$this->set( 'state', (int) ( ( ( ! $plugin->params->get( 'blog_approval', 0 ) ) || $isModerator ) ? Get::get( $array, 'published', $this->get( 'state' ), GetterInterface::INT ) : $this->get( 'state', ( $isModerator || ( ! $plugin->params->get( 'blog_approval', 0 ) ) ? 1 : 0 ) ) ) );
