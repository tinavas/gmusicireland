(function($) {
	...
	var methods = {
		init: function( options ) {
			return this.each( function () {
				...
				cbrepeat.element.bind( 'cloned.cbrepeat', function( e, oldId ) {
	...
	function addRow( row ) {
		...
		items.each( function() {
			...
			$( this ).triggerHandler( 'cloned', [ $( this ).data( 'orgId' ) ] );
	...
	function updateRepeat( cbrepeat ) {
		...
		if ( cbrepeat.settings.max ) {
			if ( row.length >= cbrepeat.settings.max ) {
				row.find( '.cbRepeatRowAdde' ).filter( function() {
					return $( this ).closest( '.cbRepeatRow' ).is( row );
				}).addClass( 'hidden' );
			} else {
