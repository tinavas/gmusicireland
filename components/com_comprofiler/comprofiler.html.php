		...
		if ( ( $user->banned != 0 ) || ( ( $user->block == 1 ) && $user->confirmed && $user->approved ) ) {
			if ( $user->banned != 0 ) {
				if ( $_CB_framework->myId() != $user->id ) {
					$_CB_framework->enqueueMessage( CBTxt::T( 'UE_USERPROFILEBANNED', 'This profile has been banned by a moderator.' ) . ( $user->bannedreason ? '<p>' . nl2br( $user->bannedreason ) . '</p>' : null ), 'error' );
				} else {
...
******************************/
	 ...
	 * @param bool          $listAll
	 */
	static function usersList( &$row, &$users, &$columns, &$fields, &$input, $search, $searchmode, $pageNav, &$myUser, &$searchableFields, &$searchValues, &$tabs, $errorMsg, $listAll = true ) {
		...
		if ( is_array( $users ) && $allowProfileLink ) foreach( $users as $k => $user ) {
			$js							.=	"cbUserURLs[$k] = '" . addslashes( $_CB_framework->userProfileUrl( (int) $user->id, false ) ) . "';";
		}
		...
		if ( $isSearching && $params->get( 'list_search_collapse', 0 ) ) {
			$isCollapsed				=	true;
		} else {
			$isCollapsed				=	false;
		}
		...
		// Base form URL:
		$baseUrl						=	$_CB_framework->rawViewUrl( 'userslist', true, array( 'listid' => (int) $row->listid, 'searchmode' => 0 ) );
		...
		$return							=	'<div class="cbUsersList cb_template cb_template_' . selectTemplate( 'dir' ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : null ) . '">'
										.		'<form action="' . $_CB_framework->rawViewUrl( 'userslist', true, array( 'listid' => (int) $row->listid ) ) . '" method="get" id="adminForm" name="adminForm" class="cb_form form-auto cbValidation">'
										...
										.			'<input type="hidden" name="Itemid" value="' . getCBprofileItemid( null, 'userslist' ) . '" />'
...
******************************/
	...
	static function registerForm( /** @noinspection PhpUnusedParameterInspection */ $option, $emailpass, $user, $postvars, $regErrorMSG = null, $stillDisplayLoginModule = false ) {
		...
		$output							=	'htmledit';
		$formatting						=	( isset( $ueConfig['use_divs'] ) && ( ! $ueConfig['use_divs'] ) ? 'tabletrs' : 'divs' );
