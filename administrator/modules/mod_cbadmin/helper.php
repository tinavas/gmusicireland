			...
			if ( ! $disabled ) {
				...
				if ( $items ) {
					...
					foreach ( $items as $item ) {
						...
						if ( $title && $link && modCBAdminHelper::checkAccess( $access ) ) {
							$return		.=			'<li' . ( $subMenu ? ' class="' . ( checkJversion( '3.0+' ) ? 'dropdown-submenu' : 'node' ) . '"' : null ) . '>'
										...
										.					'<span>' . $title . '</span>'
										.				'</a>'
										.				( $subMenu ? modCBAdminHelper::getMenuItems( $item, $subMenu, false, ( $depth + 1 ) ) : null )
										.			'</li>';
						} elseif ( $title == 'spacer' ) {
							$return		.=			'<li class="' . ( checkJversion( '3.0+' ) ? 'divider' : 'separator' ) . '"><span></span></li>';
						}
					}
