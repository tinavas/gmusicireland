 ...
 * @since       1.6
 */
class CWtagsModelCWtag extends JModelAdmin
{
	/**
	...
	/**
	 * Batch client changes for a group of cwtags.
	 *
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		$user = JFactory::getUser();
		if (!$user->authorise('core.create', 'com_cwtags.category.' . $categoryId))
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_cwtags.category.' . (int) $record->catid);
		}
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'CWtag', $prefix = 'CWtagsTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cwtags.cwtag', 'cwtag', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_cwtags.edit.cwtag.data', array());
		...
		if (empty($data))
		{
			...
			if ($this->getState('cwtag.id') == 0)
			{
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_cwtags.cwtags.filter.category_id')));
			}
		}
	...
	public function getData()
	{
		if ($this->data === null) {
			...
			$temp = (array)$app->getUserState('com_cwtags.cwtag.data', array());
			foreach ($temp as $k => $v) {
				$this->data->$k = $v;
			}
...
*/		
	...
	function delete(&$cid = array()) {
		...
		if (count( $cid )) {
			...
			$queryd = 'SELECT filename_real FROM #__cwtags_files WHERE event_id IN ( '.$cids.' )';
			$this->_db->setQuery($queryd);
			$fileObject = $this->_db->loadObjectList();
			// - - - - - - - - - - - - - 

			//Delete tags from DB
			$query = 'DELETE FROM #__cwtags_files'
			...
			// DELETE FILES ON SERVER
				foreach ($fileObject as $key => $value) {
					JFile::delete(JPath::clean($value->filename_real));
				}
