 ...
 * @since		1.5
 */
class CWtagsTableCWtag extends JTable
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__cwtags', 'id', $_db);
	...
	public function bind($array, $ignore = array())
	{
		if (isset($array['params']) && is_array($array['params'])) {
			...
			if((int) $registry->get('width', 0) < 0){
				$this->setError(JText::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', JText::_('COM_CWTAGS_FIELD_WIDTH_LABEL')));
				return false;
			}

			if((int) $registry->get('height', 0) < 0){
				$this->setError(JText::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', JText::_('COM_CWTAGS_FIELD_HEIGHT_LABEL')));
				return false;
			}
	...
	function store($updateNulls = false)
	{
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('CWtag', 'CWtagsTable');
