		...
		if ( $task == 'resendconfirmationemails' ) {
			...
			if ( $task == 'emailusers' ) {
				...
				foreach ( $users as $user ) {
					...
					if ( $simulationMode ) {
						$usernames			.=	'<div class="alert alert-info">' . CBTxt::T( 'Emails do not send in simulation mode' ) . '</div>';
					} else {
						$extraStrings		=	array();
						...
						if ( ! $cbNotification->sendFromSystem( $user, $emailSubject, $this->makeLinksAbsolute( $emailBody ), true, $mode, null, null, $attachments, $extraStrings, false, $emailFromName, $emailFromAddr, $emailReplyName, $emailReplyAddr ) ) {
							$usernames		.=	'<div class="alert alert-danger">' . CBTxt::Th( 'ERROR_SENDING_EMAIL_ERRORMSG', 'Error sending email: [ERROR_MSG]', array( '[ERROR_MSG]' => $cbNotification->errorMSG ) ) . '</div>';
							++$errors;
						}
					}
				}
