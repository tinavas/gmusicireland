	...
	<?php echo $this->tabs->startPane( 'mail_tab');
				...
				if(count($this->lists) > 10){
					...
					<!--
						function acymailing_searchAList(){
							var tableItem = document.getElementById("receiverstable");
							var filter = document.getElementById("acymailing_searchList").value.toLowerCase();
							var lists = tableItem.getElementsByClassName("acymailing_itemLine");
							for(i=0 ; i<lists.length ; i++){
								itemName = lists[i].childNodes[3].innerText.toLowerCase();
								if(document.getElementById(lists[i].childNodes[1].innerText+"listmail1").checked || itemName.indexOf(filter)>-1){
									lists[i].style.display = "table-row";
								}else{
		...
		<?php
				...
				foreach($this->lists as $row){
					...
					$classList .= ' acymailing_itemLine';
		?>
				<tr class="<?php echo "row$k $classList"; ?>">
					<td style="display:none;"><?php echo $row->listid;?></td>
