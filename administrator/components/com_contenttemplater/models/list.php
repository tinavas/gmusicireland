...
class ContentTemplaterModelList extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$filter = $this->getState('filter.button_enable_in_frontend');
		if ($filter != '')
		{
			$query->where('a.params LIKE ' . $db->q('%"button_enable_in_frontend":"' . $filter . '"%'));
		}

		$filter = $this->getState('filter.load_enabled');
		if ($filter != '')
		{
			$query->where('a.params LIKE ' . $db->q('%"load_enabled":"' . $filter . '"%'));
		}
