...
class cbTabs extends cbTabHandler
{
	...
	public function _getTabFieldsDb( $tabId, $user, $reason, $fieldIdOrName = null, $prefetchFields = true, $fullAccess = false )
	{
		...
		if ( ( ! $prefetchFields ) || ! isset( $preFetched[$preIdx] ) ) {
			...
			$_CB_database->setQuery( $sql );
			if ( $prefetchFields ) {
				$fieldsByName[$preIdx]	=	$_CB_database->loadObjectList( 'name', '\CB\Database\Table\FieldTable', array(), true );		// true means strtolower array indexes of name
				if ( ! $_CB_database->getErrorNum() ) {
					foreach ( array_keys( $fieldsByName[$preIdx] ) as $i ) {
						$fieldsByName[$preIdx][$i]->params								=	new Registry( $fieldsByName[$preIdx][$i]->params );
						$preFetched[$preIdx][(int) $fieldsByName[$preIdx][$i]->tabid][$fieldsByName[$preIdx][$i]->fieldid]	=	$fieldsByName[$preIdx][$i];
					}
				}
			} else {
