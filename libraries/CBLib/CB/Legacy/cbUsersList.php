...
class cbUsersList
{
	 ...
	 * @return string
	 */
	public static function getSorting( $listId, $userId = null, $random = 0 )
	{
		global $_CB_database;
			...
			if ( $filtering ) {
				...
				foreach ( $filtering as $filter ) {
					...
					if ( $column && $operator ) {
						...
						$value				=	$cbUser->replaceUserVars( $value, array( $_CB_database, 'getEscaped' ), false );
						...
						if ( substr( $operator, -8 ) == '||ISNULL' ) {
							$filters[]		=	'( ' . $column . ' ' . substr( $operator, 0, -8 ) . ' ' . $value . ' OR ISNULL( ' . $column . ' ) )';
						} else {
	...
	public function drawUsersList( $userId, $listId, $postData )
	{
		...
		$orderBy					=	self::getSorting( $listId, $userId, ( $limitstart ? (int) $random : 0 ) );
		...
		HTML_comprofiler::usersList( $row, $users, $columns, $fields, $input, $search, $searchMode, $pageNav, $user, $searchableFields, $searchValues, $tabs, $errorMsg, $listAll );
	}
