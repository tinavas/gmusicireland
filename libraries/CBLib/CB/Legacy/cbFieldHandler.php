...
class cbFieldHandler extends cbPluginHandler
{
	 ...
	 */
	protected function _arrayToFormat( &$field, $values, $output, $listType = ', ', $class = '' )
	{
		switch ( $output ) {
			case 'html':
			case 'rss':
	 ...
	 * @return mixed
	 */
	protected function _linksArrayToFormat( &$retArray, $output )
	{
		switch ( $output ) {
			case 'html':
			case 'rss':
				...
				foreach ( $retArray as $res ) {
					if ( isset( $res['url' ] ) ) {
						$imploded	.=	'<a href="' . cbSef( $res["url"] ) . '" title="' . htmlspecialchars( CBTxt::T( $res["tooltip"] ) ) . '">' . $res['title'] . '</a> ';
					}
				}
