...
class CBframework
{
	...
	var $_baseFramework;
	/**
	 * unix time() at page load or start
	 * @var int
	 */
	var $_now;
	/**
	...
	var $_cmsSefFunction;
	/**
	 * Redirect function of the framework
	 ...
	 * @param $editorDisplay
	 */
	function CBframework( &$baseFramework, &$aclParams, $cmsSefFunction, $cmsRedirectFunction,
						  $cbUrlRouting, $getVarFunction, &$getDocFunction, $editorDisplay )
	{
	 ...
	 * @deprecated 2.0  Use Application::MyUser()->getUserId() or Application::MyUser()->get( 'id' )
	 *
	 * @return int
	 */
	function myId( )
	...
	public function itemid( )
	{
		...
		if ( $idCache === null ) {
			if ( checkJversion() >= 2 ) {
				...
				if ( ! $idCache ) {
					$idCache		=	(int) JUri::getInstance()->getVar( 'Itemid' );
				}
			}  else {
				global $Itemid;
				$idCache			=	(int) $Itemid;
