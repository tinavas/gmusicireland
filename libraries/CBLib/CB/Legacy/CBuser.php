...
class CBuser
{
	 ...
	 * @return CBuser|null                CB User corresponding to $userAttrValue
	 */
	 ...
	 * @return string
	 */
	public function _evaluateIfs( $input )
	{
//		$regex		=	"#\[if ([^\]]+)\](.*?)\[/if\]#s";
					...
					|| ( ( count( $conditions[0] ) == 1 ) && ( in_array( $conditions[2][0], array('viewaccesslevel','gids' ) ) ) ) )	// [cb:if viewaccesslevel includes "1"] or [cb:if viewaccesslevel !includes "1"] and same for gids
				{
					...
					for ( $i = 0, $n = count( $conditions[0] ); $i < $n; $i++ ) {
						...
						if ( $field === 'viewaccesslevel' ) {
							$var	=	$user ? Application::User( (int) $user->getUserData()->id )->getAuthorisedViewLevels() : Application::MyUser()->getAuthorisedViewLevels();
						} elseif ( $user === null ) {
							$var	=	'0';
