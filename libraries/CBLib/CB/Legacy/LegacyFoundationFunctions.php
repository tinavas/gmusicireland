...
namespace CB\Legacy
{
	...
	class LegacyFoundationFunctions
	{
		...
		function __construct( ApplicationContainerInterface $di )
		{
			...
			define( '_CB_JQUERY_VERSION', '1.11.1' );		// IMPORTANT: when changing version here also change in the 2 XML installation files
			define( '_CB_JQUERY_MIGRATE', true );		// Forces jQuery Migrate plugin to load globally
			...
			$_CB_framework			=	new CBframework( $mainframe, $aclParams, $sefFunc, $cmsRedirectFunc, array( 'option' => 'com_comprofiler' ), $getVarFunction, $getDocFunction, $editorDisplay );
	...
	function cbimport( $lib ) {
		...
		if ( ! isset( $imported[$lib] ) ) {
			...
			if ( $pathAr[0] == 'language' ) {
				...
				if ( in_array( $pathAr[1], array( 'front', 'all' ) ) && ( ! isset( $imported['language.front'] ) ) ) {
					...
					if ( file_exists( $langPath . '/' . $lang . '/' . $filename ) ) {
						CBTxt::import( $langPath, $lang, $filename );
					}
				}

				if ( in_array( $pathAr[1], array( 'admin', 'all' ) ) && ( ! isset( $imported['language.admin'] ) ) ) {
				...
				// If we're a specific language file lets just fallback to looking for that specific file:
				if ( ! in_array( $pathAr[1], array( 'front', 'admin', 'all' ) ) ) {
