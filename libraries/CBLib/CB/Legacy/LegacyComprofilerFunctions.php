...
// 	if(preg_match('/gallery\//i',$avatar)==false && is_file($_CB_framework->getCfg('absolute_path').'/images/comprofiler/'.$avatar)) {
	...
	function activateUser( &$user, $ui, $cause, $mailToAdmins = true, $mailToUser = true, $triggerBeforeActivate = true ) {
		...
		if ( $cause == 'UserConfirmation' && $user->approved == 0) {
			...
			$msg = array(
				...
				'emailAdminMessage'	=> array( 'sys' => CBTxt::T( 'UE_REG_ADMIN_MSG', "A new user has signed up at [SITEURL]. at [SITEURL].\nThis email contains their details\n\nName - [NAME]\nE-mail - [EMAILADDRESS]\nUsername - [USERNAME]\n\n\nPlease do not respond to this message as it is automatically generated and is for information purposes only\n" ) )
			);
			$savedLanguage				=	CBTxt::setLanguage( $user->getUserLanguage() );
	...
	function cbFormatDate( $date, $serverTimeOffset = true, $showTime = true, $dateFormatOverride = null, $timeFormatOverride = null, $offsetOverride = null ) {
		...
		if ( ( $date != '' ) && ( $date != null ) && ( $date != '0000-00-00 00:00:00' ) && ( $date != '0000-00-00' ) ) {
			...
			if ( ( $showTime === 'timeago' ) || ( $showTime === 'exacttimeago' ) ) {
				...
				if ( ! $JS_loaded++ ) {
					$messages			=	array(	'future' => addslashes( CBTxt::T( 'TIMEAGO_FUTURE', 'in %s' ) ),
													'past' => addslashes( CBTxt::T( 'TIMEAGO_PAST', '%s ago' ) ),
													'seconds' => addslashes( CBTxt::T( 'TIMEAGO_LESS_THAN_A_MINUTE', 'less than a minute' ) ),
													'minute' => addslashes( CBTxt::T( 'TIMEAGO_ABOUT_A_MINUTE', 'about a minute' ) ),
													'minutes' => addslashes( CBTxt::T( 'TIMEAGO_N_MINUTES', '%d minutes' ) ),
													'hour' => addslashes( CBTxt::T( 'TIMEAGO_ABOUTE_ONE_HOUR', 'about an hour' ) ),
													'hours' => addslashes( CBTxt::T( 'TIMEAGO_ABOUT_N_HOURS', 'about %d hours' ) ),
													'day' => addslashes( CBTxt::T( 'TIMEAGO_A_DAY', 'a day' ) ),
													'days' => addslashes( CBTxt::T( 'TIMEAGO_N_DAYS', '%d days' ) ),
													'month' => addslashes( CBTxt::T( 'TIMEAGO_ABOUT_A_MONTH', 'about a month' ) ),
													'months' => addslashes( CBTxt::T( 'TIMEAGO_N_MONTHS', '%d months' ) ),
													'year' => addslashes( CBTxt::T( 'TIMEAGO_ABOUT A_YEAR', 'about a year' ) ),
													'years' => addslashes( CBTxt::T( 'TIMEAGO_N_YEARS', '%d years' ) )
												);

					$options			=	array( 'strings' => $messages );

					$_CB_framework->outputCbJQuery( "$( '.cbDateTimeago' ).cbtimeago(" . json_encode( $options ) . ");", 'cbtimeago' );
				}
				...
				$dateFormat				=	( $dateFormatOverride ? $dateFormatOverride : ( CBTxt::T( 'UE_DATE_FORMAT', '' ) != '' ? CBTxt::T( 'UE_DATE_FORMAT', '' ) : $ueConfig['date_format'] ) );

				if ( $showTime && ( strlen( $date ) > 10 ) ) {
					$dateFormat			.=	( $timeFormatOverride ? $timeFormatOverride : ( CBTxt::T( 'UE_TIME_FORMAT', '' ) != '' ? CBTxt::T( 'UE_TIME_FORMAT', '' ) : ' ' . ( isset( $ueConfig['time_format'] ) ? $ueConfig['time_format'] : 'H:i:s' ) ) );
				}
	...
	function initToolTip( /** @noinspection PhpUnusedParameterInspection */ $obsoleteUi = 0, $width = 250 ) {
		...
		if ( ! $cache ) {
			...
			$js				=	"$( window ).load( function() {"
							.		"$( '.cbTooltip,[data-hascbtooltip=\"true\"]' ).on( 'cbtooltip.init.before', function( e, cbtooltip ) {"
							.			"if ( cbtooltip.element.parents( '.cbTooltip,[data-hascbtooltip=\"true\"]' ).length ) {"
							.				"cbtooltip.settings.init = false;"
							.			"}"
							.		"}).cbtooltip({"
							.			"classes: 'cb_template cb_template_" . addslashes( selectTemplate( 'dir' ) ) . "',"
							.			"buttonClose: '" . addslashes( CBTxt::T( 'TOOLTIP_CLOSE CLOSE', 'Close' ) ) . "'"
							.			( $width ? ",width: " . (int) $width : null )
							.		"}).on( 'cbtooltip.render', function( e, cbtooltip, event, api ) {"
	...
	function cbTooltip( $ui, $tooltip, $title = null, $size = null, $image = null, $html = '<span class="fa fa-info-circle"></span>', $href = null, $attributes = null, $imageAttributes = null ) {
		...
		if ( $size ) {
			if ( is_array( $size ) ) {
				if ( count( $size ) > 1 ) {
					$width			=	( $size[0] == 'auto' ? false : ( (int) $size[0] ? (int) $size[0] . ( strpos( $size[0], '%' ) !== false ? '%' : null ) : null ) );
					$height			=	( $size[1] == 'auto' ? false : ( (int) $size[1] ? (int) $size[1] . ( strpos( $size[1], '%' ) !== false ? '%' : null ) : null ) );
				} else {
					$width			=	( $size[0] == 'auto' ? false : ( (int) $size[0] ? (int) $size[0] . ( strpos( $size[0], '%' ) !== false ? '%' : null ) : null ) );
					$height			=	null;
				}
			} else {
				$width				=	( $size == 'auto' ? false : ( (int) $size ? (int) $size . ( strpos( $size, '%' ) !== false ? '%' : null ) : null ) );
				$height				=	null;
			}
		} else {
			$width					=	null;
		...
		$toolTipAttributes			=	( $class ? ' class="' . htmlspecialchars( $class ) . '"' : null )
			...
			.	( $width !== null ? ' data-cbtooltip-width="' . $width . '"' : null )
			.	( $height !== null ? ' data-cbtooltip-height="' . $height . '"' : null )
					...
					if ( $count ) {
						if ( $userId ) {
							$query				=	'UPDATE ' . $_CB_database->NameQuote( '#__comprofiler' )
								...
								.	', ' . $_CB_database->NameQuote( 'message_last_sent' ) . ' = NOW()'
								.	"\n WHERE " . $_CB_database->NameQuote( 'id' ) . " = " . (int) $userId;
							$_CB_database->setQuery( $query );
							$_CB_database->query();
						} else {
							$_CB_framework->setUserState( 'cb_message_number_sent', 1 );
							$_CB_framework->setUserState( 'cb_message_last_sent', date( 'Y-m-d H:i:s', $_CB_framework->now() ) );
						}
					}
				}
			} else {
		...
		<div class="cbTeamCredits cb_template cb_template_<?php echo selectTemplate( 'dir' ); ?>">
			<div class="container-fluid">
				...
				<div class="row">
					...
					?>
					<p><strong>Software: Copyright 2004-2014 joomlapolis.com. This component is released under the GNU/GPL version 2 License. All copyright statements must be kept. Derivate work must prominently duly acknowledge original work and include visible online links. Official site:</strong></p>
	...
	function cbGetMimeMap() {
		$map	=	array(	'ez' => 'application/andrew-inset', 'aw' => 'application/applixware', 'atom' => 'application/atom+xml', 'atomcat' => 'application/atomcat+xml',
							   ...
							   'mp2' => 'audio/mpeg', 'mp2a' => 'audio/mpeg', 'mp3' => 'audio/mpeg', 'm2a' => 'audio/mpeg',
							   'm3a' => 'audio/mpeg', 'oga' => 'audio/ogg', 'ogg' => 'audio/ogg', 'spx' => 'audio/ogg',
							   ...
							   'mjp2' => 'video/mj2', 'mp4' => 'video/mp4', 'mp4v' => 'video/mp4', 'mpg4' => 'video/mp4',
							   'mpeg' => 'video/mpeg', 'mpg' => 'video/mpeg', 'mpe' => 'video/mpeg', 'm1v' => 'video/mpeg',
							   ...
							   'fli' => 'video/x-fli', 'flv' => 'video/x-flv', 'm4v' => 'video/x-m4v', 'mkv' => 'video/x-matroska',
