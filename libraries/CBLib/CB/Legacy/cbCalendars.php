...
class cbCalendars
{
	 ...
	 */
	public function cbAddCalendar( $name, $label = null, $required = false, $value = null, $readOnly = false, $showTime = false, $minYear = null, $maxYear = null, $attributes = null )
	{
		...
		if ( $readOnly ) {
			...
			$return						=	'<input type="text" name="' . htmlspecialchars( $name ) .'" id="' . htmlspecialchars( $inputId ) . '" value="' . htmlspecialchars( $value ) . '" class="cbDatePicker form-control' . ( $required ? ' required' : null ) . '"' . ( $readOnly ? ' disabled="disabled"' : null ) . ( $label ? ' title="' . htmlspecialchars( $label ) . '"' : null ) . ( trim( $attributes ) ? ' ' . $attributes : null ) . ' />'
										.	$return;
		}
