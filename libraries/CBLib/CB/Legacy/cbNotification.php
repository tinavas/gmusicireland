...
class cbNotification
{
	...
	private function sendThatMail( $rowTo, $rowFrom, $subject, $message, $revealEmail, $replaceVariables, $mode, $cc, $bcc, $attachment, $extraStrings, $replyToName, $fromNameHidden )
	{
		...
		if ( $revealEmail ) {
			if ( isset( $ueConfig['allow_email_replyto'] ) && ( $ueConfig['allow_email_replyto'] == 2 ) ) {
				...
				$rowFrom->name				=	$this->defaultFromName();
			} else {
				$replyToEmail				=	null;
				$replyToName 				=	null;
			}
			...
			$rowFrom->email					=	$ueConfig['reg_email_from'];
			if ( $fromNameHidden ) {
				$rowFrom->name				=	$fromNameHidden;
			}

			$toUserLanguage					=	CBuser::getInstance( (int) $rowTo->id )->getUserData()->getUserLanguage();
			$savedLanguage					=	CBTxt::setLanguage( $toUserLanguage );

			$rowFrom->name					=	CBTxt::T( 'EMAIL_NOTE_NOTIFICATIONS_AT_SITENAME', 'Notifications at [sitename]', array( '[sitename]' => $_CB_framework->getCfg( 'sitename' ) ) );
		...
		return $this->_sendEmailMSG( $rowTo, $rowFrom, $replyToName, $replyToEmail, $subject, $message, $revealEmail, $mode, $cc, $bcc, $attachment );
	}
	 ...
	 * @return boolean                            Result
	 */
	private function _sendEmailMSG( $toUser, $fromUser, $replyToName, $replyToEmail, $subject, $message, $addPrefix = false, $mode = 0, $cc = null, $bcc = null, $attachment = null )
	{
		...
		if ( $addPrefix ) {
			...
			if ( isset( $ueConfig['allow_email_replyto'] ) && ( $ueConfig['allow_email_replyto'] == 2 ) ) {
				$preMsg		.=	$this->_replaceVariables(
					"\r\n\r\n" . CBTxt::T( 'EMAIL_NOTE_WHEN_REPLYING_CHECK_ADDRESS', 'When replying, please check carefully that the email address of [formatname] is [email].' ),
					$fromUser, $mode
				);
			}
			...
			$fromUser->name		=	getNameFormat( $fromUser->name, $fromUser->username, $ueConfig['name_format'] ) . ' @ ' . $_CB_framework->getCfg( 'sitename' );
