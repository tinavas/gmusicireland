...
//$sqlUpgrader->setDryRun( true );
		...
		if ( $libraries_element !== false ) {
			foreach( $libraries_element->children() as $library ) {
				...
				$saveElement						=	$this->elementDir( $_CB_framework->getCfg( 'absolute_path' ) . '/libraries/CBLib/CB/Plugin/' . $library->attributes( 'name' ) . '/' );
