...
class XmlQuery {
	...
	public function addWhere( $fieldName, $operator, $fieldValue, $type ) {
		...
		if ( substr( $operator, -8 ) == '||ISNULL' ) {
			...
			$this->where[]	=	$this->_currentTableAs . '.`' . $this->_db->getEscaped( $fieldName ) . "` " . $operator . " " . $value;		//BM maintableAs
		}
	}
...
//EXIT;
	...
	public function process_filter( $filter, $filterValuesArray, $valueType )
	{
		...
		$where					=	$filter->getElementByPath( 'data/where');
		if ( $where ) {
			...
			if ( $filterValuesArray['valuefield'] && $filterValuesArray['operator'] && $filterValuesArray['internalvalue'] && $valueType ) {
		...
		$joinkeys			=	$filter->getElementByPath( 'data/joinkeys');
		if ( $joinkeys ) {
			$data			=	$filter->getElementByPath( 'data');
			$this->_changeJoinType( $data->attributes( 'name' ), $joinkeys );
	 ...
	 * @param  SimpleXMLElement  $joinkeys  'inner'
	 */
	protected function _changeJoinType( $name, SimpleXMLElement $joinkeys ) {
		if ( isset( $this->leftJoinedFieldsTable[$name] ) && isset( $this->leftJoinArray[ $this->leftJoinedFieldsTable[$name] ] ) ) {
			$joinKeyword	=	$this->computeJoinKeyword( $joinkeys );
			if ( $joinKeyword != 'LEFT JOIN' ) {
				$previous	=	$this->leftJoinArray[ $this->leftJoinedFieldsTable[$name] ];
				if ( substr( $previous, 0, 10 ) == 'LEFT JOIN ' ) {
					$this->leftJoinArray[ $this->leftJoinedFieldsTable[$name] ]	=	$joinKeyword . ' ' . substr( $previous, 10 );
					$this->joinsNeededForCount	=	true;
				}
			}
		}
	...
	protected function process_column( $column, $filterValuesArray = null, $returnExpression = false, $doGroupBy = true ) {
		...
		if ( ( $colVal !== false ) && ( ( $colValueType != 'sql:formula' ) || ( $colVal !== null ) ) ) {		// in case of formula children, the condition is embedded in joins, except for reverse join traversals.
			...
			$expression			=	$expression
								.	' ' . $operator . ' ';
			if ( in_array( strtolower( $operator ), array( 'in', 'not in' ) ) ) {
				$expression		.=	'(' . $colVal . ')';
			} else {
				$expression		.=	$colVal;
			}
	...
	private function get_field( $field ) {
		...
		$cnt_resultfield	=	$field->attributes( 'resultfield' );
		$fieldsArray		=	array();
		...
		if ( ! $this->_has_data_children( $field ) ) {
			...
			$fieldsArray[( $cnt_as ? $cnt_as : $cnt_name )]		=	( $cnt_resultfield == 'true' ? '' : $this->_db->getEscaped( $tableAs ) . '.' ) . '`' . $this->_db->getEscaped( $cnt_name ) . '`'
				.	( $cnt_as ? ' AS `' . $cnt_as . '`' : '' );
...
//		$xmlsql->_table				=	$subqueryTable;
			...
			if  ( $cnt_table == $this->_table ) {
				...
				if ( ( ! $cnt_valtype ) || ( $cnt_valtype == 'sql:field' ) ) {
					...
					$this->leftJoinArray[$this->tableAs]	= $joinKeyword . $this->_db->NameQuote( $cnt_table ) . ' AS ' . $this->tableAs
						. ' ON ' . $this->tableAs . '.' . implode( ' AND ', $subFormula );	// small shortcut allowing only one level for now
				...
				$this->leftJoinedFieldsTable[( $cnt_as ? $cnt_as : $cnt_name )]		=	$this->tableAs;

				$formula		=	/* $this->tableAs . '.' . */ '`' . $cnt_val . '`'											// small shortcut allowing MORE THAN one level for now
					.	'='
	...
	protected function _addGetJoinAs( $data, $subFormula = null ) {
			...
			&& ( ( $cnt_table != $this->_table ) || ( $cnt_key && $cnt_val ) || $cnt_joinkeys ) )
		{
			...
			if ( isset( $this->leftJoindTableKeyValueArray[$cnt_table][$cnt_key][$cnt_val][$cnt_valtype . $subFormula] ) ) {
				$tableAs				=	$this->leftJoindTableKeyValueArray[$cnt_table][$cnt_key][$cnt_val][$cnt_valtype . $subFormula];
			} else {
				$this->incrementTableAs();
				...
				$this->leftJoindTableKeyValueArray[$cnt_table][$cnt_key][$cnt_val][$cnt_valtype . $subFormula]	=	$this->tableAs;
