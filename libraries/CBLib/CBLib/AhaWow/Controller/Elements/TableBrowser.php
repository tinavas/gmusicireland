			...
			foreach ( $row->children() as $o ) {
				...
				if ( $o->getName() == $type ) {
					...
					if ( $o->attributes( 'operator' ) ) {
						...
						$items[$type.'_'.$o->attributes( 'name' )]['operator']		=	'=';
					}
...
EOT
					...
					*/
/*
 * The following is now computed into the base type in RegistryEditView::resolveXmlParamType() before landing here:
				} elseif ( substr( $o->attributes( 'type' ), 0, 4 ) == 'xml:' ) {
					$xmlType	=	substr( $o->attributes( 'type' ), 4 );
					...
					if ( $typeModel ) {
						...
						for ( $i = 0; $i < 100; $i++ ) {
							if ( substr( $root->attributes( 'base' ), 0, 4 ) == 'xml:' ) {
								...
								if ( ! $root ) {
									$selObj				=	new \stdClass();
									$selObj->value		=	10;
									$selObj->text		=	'Missing type definition of ' . $root->attributes( 'type' );
									$fieldValuesInDb[]	=	$selObj;
									echo $selObj->text;
									break;
								}
						...
						if ( $i == 99 ) {
							echo 'Error: recursion loop in XML type definition of ' . $o->getName() . ' ' . $o->attributes( 'name' ) . ' type: ' . $o->attributes( 'type' );
							exit;
						}
						$basetype					=	$root->attributes( 'base' );
						if ( ! $valueType ) {
							$valueType				=	$root->attributes( 'valuetype' );
						}
						$levelModel		=	$o;
						for ( $i = 0; $i < 100; $i++ ) {
							switch ( $root->attributes( 'base' ) ) {
								case 'list':
								case 'multilist':
								case 'radio':
								case 'checkbox':
									...
									foreach ( $levelModel->children() as $option ) {
										...
										if ( $option->getName() == 'option' ) {
											...
											if ( $option->attributes( 'index' ) ) {
												$selObj->internalValue		=	$option->attributes( 'value' );
												$selObj->value				=	$option->attributes( 'index' );
											} else {
												$selObj->value				=	$option->attributes( 'value' );
											}
											if ( $option->attributes( 'operator' ) ) {
												$selObj->operator			=	$option->attributes( 'operator' );
											}
											$selObj->text					=	$option->data();
											$fieldValuesInDb[]				=	$selObj;
								...
								default:
									$selObj				=	new \stdClass();
									$selObj->value		=	1;
									$selObj->text		=	'Unknown base type ' . $root->attributes( 'base' ) . ' in XML';
									$fieldValuesInDb[]	=	$selObj;
									echo $selObj->text;
									break;
							}
							if ( ( $levelModel->getName() == $o->getName() ) && ( $levelModel->attributes( 'name' ) == $o->attributes( 'name' ) ) && ( substr( $levelModel->attributes( 'type' ), 0, 4 ) == 'xml:' ) ) {
						...
						$selObj				=	new \stdClass();
						$selObj->value		=	10;
						$selObj->text		=	'Missing type def. of ' . $o->attributes( 'type' );
						$fieldValuesInDb[]	=	$selObj;
						echo $selObj->text;
					}
*/
		...
		foreach ( $rows as $name => $value ) {
			...
			if ( $postedValueRaw !== null ) {
				...
				if ( ( $rows[$name]['type'] != 'field_show_only_if_selected' ) && $rows[$name]['selectValues'] ) {
					...
					if ( is_array( $postedValueRaw ) ) {
						$rows[$name]['value']					=	cbToArrayOfInt( $postedValueRaw );
					...
					} elseif ( $rows[$name]['basetype'] == 'datetime' ) {
						if ( isset( $_POST[$this->name] ) ) {
							/** @var SimpleXMLElement $xml */
							$xml								=	$rows[$name]['xml'];
							$showTime							=	$xml->attributes( 'showtime' );

							if ( $showTime === null ) {
								$showTime						=	true;
							} else {
								$showTime						=	( ( $showTime == 'false' ) || ( $showTime == '0' ) ? false : true );
							}

							$dateFormat							=	$xml->attributes( 'dateformat' );
							$timeFormat							=	$xml->attributes( 'timeformat' );

							if ( $showTime ) {
								$format							=	array( $dateFormat . $timeFormat, 'Y-m-d H:i:s' );
							} else{
								$format							=	array( $dateFormat, 'Y-m-d' );
							}

							$value								=	$_CB_framework->getUTCDate( $format, $postedValue );
						} else {
							// it's from a GET, so it came from URL in the SQL format:
							$value								=	$postedValue;
						}
	...
	protected function _performTableActions() {
		...
		$cid			=	cbGetParam( $_REQUEST[$this->name], 'idcid', array( 0 ) );
		if ( $cid === array( 0 ) ) {
			$cid		=	array();
		}
		if (!is_array( $cid )) {
		...
		switch ( $task_parsed[0] ) {
			...
			case 'editrows':
				if ( $this->listFieldsRows ) {
					...
					$url			=	$baseUrl . '&table=' . $this->_tableBrowserModel->attributes( 'name' ) . '&action=editrow&' . urlencode( $field ) . '=' . (int) $cid[0];
			...
			case 'deleterows':
				if ( $this->listFieldsRows ) {
					...
					$dataModelClass			=	$this->class;
					foreach ( $cid as $id ) {
						$dataModelValue		=	(int) $id;
						$row				=	$this->createLoadClass( $dataModelClass, $dataModelValue );
						if ( ! $row ) {
							$_CB_framework->enqueueMessage( CBTxt::T( 'No row data found' ), 'error' );
							return;
						}
						if ( $row->canDelete( (int) $id ) ) {
			...
			case 'action':
				if ( $this->listFieldsRows ) {
					...
					if ( isset( $task_parsed[1] ) ) {
						...
						$url			=	$baseUrl . '&table=' . $this->_tableBrowserModel->attributes( 'name' ) . '&action=' . urlencode( $task_parsed[1] ) . '&' . urlencode( $field ) . '=' . (int) $cid[0];
