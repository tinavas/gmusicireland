...
class ActionController {
	 ...
	 *
	 * @return array  array( 'task' => string, 'plugin' => string, 'tabid' => null|int )
	 */
	public function getBaseOptions( )
	{
		global $_CB_framework, $_PLUGINS, $_REQUEST;
		if ( $_CB_framework->getUi() == 2 ) {

			$plugin				=	$_PLUGINS->getPluginObject();
			if ( $plugin ) {
				$pluginOptions	=	array(	'view'		=>	cbGetParam( $_REQUEST, 'view' ),
	...
	public function drawView( /** @noinspection PhpUnusedParameterInspection */ $option, $action, $element, $mode )
	{
		...
		if ( ! $found ) {
			...
			foreach ( $adminActionsModel->children() as $actionsModel ) {
				...
				if ( $request ) {
					...
					$actionPath							=	 array();
					for ( $i = 0, $n = count( $requests ); $i < $n; $i++ ) {
						$actionPath[$requests[$i]]		=	cbGetParam( $_REQUEST, $requests[$i] );
		...
		foreach ($parametersNames as $paraNam ) {
			...
			if ( strpos($paraNam, '[' ) === false ) {
				if ( trim( $paraNam ) ) {
					$parametersValues[$paraNam]			=	$this->input->get( $paraNam, '', GetterInterface::COMMAND );
				}
			} else {
				$matches								=	null;
				preg_match_all( '/(.*)(?:\[(.*)\])+/', $paraNam, $matches );
				if ( is_array( $matches ) && ( count( $matches ) >= 3 ) && ( count( $matches[2] ) >= 1 ) ) {
					$parametersValues[$paraNam] 		=	$this->input->get( $matches[1][0] . '.' . $matches[2][0], null, GetterInterface::COMMAND );
				}
			}
		}
		$keyValues										=	array();
							...
							if ( strpos( $dataModelClass, '::' ) === false ) {
								...
								if ( is_callable( $dataModelSingleton ) ) {
									if ( is_callable( array( $dataModelSingleton[0], 'getInstance' ) ) ) {
										...
										$rows			=	call_user_func_array( array( $instance, $dataModelSingleton[1] ), array( (int) $dataModelValue ) );
									} else {
										$rows			=	call_user_func_array( $dataModelSingleton, array( (int) $dataModelValue ) );
									}
								} else {
...
*/
					...
					$cbprevstate	=	stripslashes( cbGetParam( $_REQUEST, 'cbprevstate', null ) );

					$params			=	new RegistryEditController( $this->input, $this->db, new Registry(), $viewModel, $element, $context->getPluginObject() );

					// gets data back from a POST ( > 1 and not > 0 to workaround a bug in Joobi bot which puts 'option' into $_POST by error: all our spoofchecks have at least 2 post items)
					// also when registering and then directly editing the invoiving address we are in post mode, but not on the right form, and the spoofcheck fails because of different form: thus the check of plugin:
					$displayData	=	$this->bindInput( $viewMode, $data, $ui );
						...
						case 'params':
							if ( $mode == 'edit' ) {
								if ( ( $viewMode == 'edit' ) || ( $viewMode == 'show' ) ) {
									...
									if ( $ui == 2 ) {
										ActionViewAdmin::editPluginView( $options, $actionPath, $viewModel, $displayData, $params, $context->getPluginObject(), $viewTypeMode, $cbprevstate );
										$settings						=	null;
										$html							=	null;
									} else {
									...
									if ( $ui == 2 ) {
										...
										$toolbarMenu						=	$viewModel->getElementByPath( 'toolbarmenu' );
										if ( $toolbarMenu ) {
											$controller						=	new DrawController( $this->input, $viewModel, $element->getElementByPath( 'actions' ), $options );
											...
											foreach ( $toolbarMenu->children() as $menu ) {
												$menuLink					=	$menu->attributes( 'link' );
												$menuAccess					=	true;
												$link						=	null;

												if ( $menuLink ) {
													$link					=	$controller->drawUrl( $menuLink, $menu, $displayData, null );
	 ...
	 */
	protected function bindInput( $viewMode, $data, $ui ) {

		// Todo: use $this->_di->getInput()

		// gets data back from a POST ( > 1 and not > 0 to workaround a bug in Joobi bot which puts 'option' into $_POST by error: all our spoofchecks have at least 2 post items)
		// also when registering and then directly editing the invoiving address we are in post mode, but not on the right form, and the spoofcheck fails because of different form: thus the check of plugin:
		if ( ( $viewMode == 'edit' ) && ( count( $_POST ) > 1 ) ) {
			...
			*/
			...
			if ( is_object( $data ) && method_exists( $data, 'bind' ) && method_exists( $data, 'check' ) ) {
				...
				$this->recursiveMultiSelectFix( $postArray );

				if ( $ui == 2 ) {
					$treatGPCmagic		=	true;
				} else {
					foreach ( array_keys( $postArray ) as $k ) {
						$postArray[$k]	=	stripslashes( cbGetParam( $postArray, $k ) );
					}
					$treatGPCmagic		=	false;
				}
				if ( ! $displayData->bind( $postArray, '', null, $treatGPCmagic ) ) {
...
//				}
	 ...
	 *
	 * @param  array  $postArray  In and Out (modified)
	 * @return void
	 */
	protected function recursiveMultiSelectFix( &$postArray )
	{
		foreach ( $postArray as &$v ) {
			if ( is_array( $v ) ) {
				if ( ( count( $v ) == 0 ) || ( isset( $v[0] ) && ( ! is_array( $v[0] ) ) ) ) {
					...
					$this->recursiveMultiSelectFix( $v );
				}
			}
		}
	}
	...
	protected function savePluginView( $options, $actionPath, $keyValues, $parametersValues, $viewModel, $data, $params, &$mode, $dataModelType, $plugin, $dataModel, $pluginParams, $cbprevstate, $ui )
	{
		...
		// Lets establish the $_POST array to a different variable so we don't alter the real $_POST array:
		$postArray		=	$_POST;
		...
		switch ( $dataModelType ) {
			...
			case 'sql:field':			// <data name="params" type="sql:field" table="#__cbsubs_config" class="cbpaidConfig" key="id" value="1" valuetype="sql:int" />
				...
				if ( $dataModelValue ) {		// if ( (int) cbGetParam( $_POST, $dataModelKey, 0 ) ) {		//TBD LATER: what to do if no param row with id=1 ?
