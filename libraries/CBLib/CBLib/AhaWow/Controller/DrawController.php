...
class DrawController {
	...
	function orderbyfields() {
		...
		return moscomprofilerHTML::selectList( $orderbyOptions, $this->fieldName( 'orderby' ), 'class="form-control" onchange="' . htmlspecialchars( $orderbyJS ) . '"', 'value', 'text', $this->fieldValue( 'orderby' ) );
	}
	...
	function drawUrl( $cbUri, SimpleXMLElement $sourceElem, $data, /** @noinspection PhpUnusedParameterInspection */ $id, $htmlspecialchars = true, $inPage = true ) {
		...
		if ( cbStartOfStringMatch( $url, 'index.php' ) ) {
			...
			if ( count( $this->_filters ) > 0 ) {
				foreach ( $this->_filters as $k => $v ) {
					...
					if ( ( $v['value'] != $v['default'] ) && ( ! in_array( $filterName, $parametersNames ) ) ) {
						$this->addFilterValues( $parametersNames, $parametersValues, $filterName, $v['value'] );
					}
				}
			}
			...
			// add current orderby string, if any:
			$orderbyName					=	$this->fieldName( 'orderby' );
			$orderbyValue					=	$this->fieldValue( 'orderby' );
			if ( $orderbyValue && ( ! in_array( $orderbyName, $parametersNames ) ) ) {
				$parametersNames[]			=	$orderbyName;
				$parametersValues[]			=	"'" . $orderbyValue . "'";
			}
	 ...
	 */
	protected function addFilterValues( &$parametersNames, &$parametersValues, $filterName, $values )
	{
		if ( ! is_array( $values ) ) {
		...
		foreach ( $values as $name => $value ) {
			$subFilterName		=	$filterName . '[' . $name . ']';
			$this->addFilterValues( $parametersNames, $parametersValues, $subFilterName, $value );
		}
	}
