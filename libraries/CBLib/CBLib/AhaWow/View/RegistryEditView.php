...
class RegistryEditView {
			...
			if ( $this->_jsselect2 ) {
				...
				if ( ! $select2++ ) {
					$js			=	"$( '.cbSelect' ).cbselect({"
								.		"formatNoMatches: '" . addslashes( CBTxt::T( 'No matches found.' ) ) . "',"
								.		"formatSearching: '" . addslashes( CBTxt::T( 'Searching...' ) ) . "',"
								.		"formatInputTooShort: '" . addslashes( CBTxt::T( 'Search input too short.' ) ) . "',"
								.		"formatInputTooLong: '" . addslashes( CBTxt::T( 'Search input too long.' ) ) . "',"
								.		"formatSelectionTooBig: '" . addslashes( CBTxt::T( 'You cannot select any more choices.' ) ) . "',"
								.		"formatLoadMore: '" . addslashes( CBTxt::T( 'Loading more results…' ) ) . "',"
	...
	function phpCleanType( $fieldValue, $type, $element, $leftRight ) {
		...
		switch ( $typeArray[0] ) {
			...
			case 'request':
				global $_REQUEST;
				$fieldValue		=	cbGetParam( $_REQUEST, $fieldValue );
				break;
			case 'get':
				global $_GET;
				$fieldValue		=	cbGetParam( $_GET, $fieldValue );
				break;
			case 'post':
				global $_POST;
				$fieldValue		=	cbGetParam( $_POST, $fieldValue );
				break;
			case 'cookie':
				global $_COOKIE;
				$fieldValue		=	cbGetParam( $_COOKIE, $fieldValue );
				break;
			case 'session':
				global $_SESSION;
				$fieldValue		=	cbGetParam( $_SESSION, $fieldValue );
				break;
			case 'server':
				global $_SERVER;
				$fieldValue		=	cbGetParam( $_SERVER, $fieldValue );
				break;
			case 'env':
				global $_ENV;
				$fieldValue		=	cbGetParam( $_ENV, $fieldValue );
				break;
			default:
	...
	*/
					...
					if ( $dataModelClass && $methodName ) {
						...
						if ( strpos( $dataModelClass, '::' ) === false ) {
							if ( class_exists( $dataModelClass ) ) {
								...
								if ( $dataModelValue ) {
									$viewerClass->load( (int) $dataModelValue );
								}
							} else {
	...
	function renderParam( &$param, $control_name = 'params', $view = true, $htmlFormatting = 'table' ) {
		...
		if ( $name ) {
			...
			} elseif ( ( $type == 'private' ) && ( ! ( $this->getModelOfData() instanceof RegistryInterface ) ) && ! isset( $this->getModelOfData()->$name ) ) {
	...
	function _form_tag( $name, $value, &$node, $control_name ) {
		if ( $this->_view ) {
			...
			$this->_list_options( $name, $node, $control_name, $options, $node->children(), false, null, true );
			...
			$attributes					=	' data-cbselect-tags="' . htmlspecialchars( json_encode( $data ) ) . '"';

			if ( $node->attributes( 'valuedescription' ) || $node->attributes( 'description' ) ) {
				$select2Id				=	'#s2id_' . htmlspecialchars( moscomprofilerHTML::htmlId( $this->control_name( $control_name, $name ) ) );

				$attributes				.=	' data-cbtooltip-open-target="' . $select2Id . '" data-cbtooltip-close-target="' . $select2Id . '" data-cbtooltip-position-target="' . $select2Id . '"';
			}

			return $this->textfield( $name, $value, $node, $control_name, array( 'cbSelect' ), $attributes );
		}
	}
	...
	function _form_folderlist( $name, $value, &$node, $control_name ) {
		...
		if ( $this->_view ) {
			...
			$this->_list_options_default( $node, $options, $value );
			$this->_list_options( $name, $node, $control_name, $options, $node->children(), true, $value );
			$this->_list_options_data( $node, $options, $folders );
	...
	function _form_datetime( $name, $value, &$node, $control_name ) {
		...
		if ( $this->_view ) {
			...
			$content			=	$calendars->cbAddCalendar( $this->control_name( $control_name, $name ), '', $required, $value, false, (bool) $showTime, $minYear, $maxYear, $attributes );
		}
