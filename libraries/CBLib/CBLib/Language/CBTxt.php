...
class CBTxt
{
	 ...
	 *
	 * @param  string|null  $language  New ISO language (ISO 639-1 language code, an underscore (_), then the ISO 3166-1 alpha-2 country code: e.g. 'en-GB'), NULL or '' doesn't change it.
	 ...
	 * @param  string  $language  ISO language (ISO 639-1 language code, an underscore (_), then the ISO 3166-1 alpha-2 country code: e.g. 'en-GB') (which is also the name of the language folder)
	...
	public static function import( $langPath, $language, $filename, $fallback = true ) {
		...
		if ( ! isset( static::$self->importedLangPathFiles[$language][$langPath][$filename] ) ) {
			...
			if ( file_exists( $langPath . '/' . $language . '/' . $filename ) ) {
				/** @noinspection PhpIncludeInspection */
				$file			=	$langPath . '/' . $language . '/' . $filename;
			} elseif ( $fallback && file_exists( $langPath . '/default_language/' . $filename ) ) {
				/** @noinspection PhpIncludeInspection */
				$file			=	$langPath . '/default_language/' . $filename;
			...
			if ( count( static::$self->importedLangPathFiles ) > 1 ) {
			...
			static::$self->importedLangPathFiles[$language][$langPath][$filename]	=	$loaded;
