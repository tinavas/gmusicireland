...
class CBPHPMailer		//BB: Line changed for CB library structure.
{
     ...
     * @type CBSMTP							//BB changed to CBSMTP own class
     */
     ...
     * @return CBSMTP							//BB adjusted from SMTP to CBSMTP
     */
    public function getSMTPInstance()
    {
        if (!is_object($this->smtp)) {
            $this->smtp = new CBSMTP;			//BB using CBSMTP
        }
        return $this->smtp;
    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new CBphpmailerException($this->lang('signing') . ' OpenSSL extension missing.');
                }
        ...
        try {
            ...
            if ($magic_quotes) {
                if (version_compare(PHP_VERSION, '5.3.0', '<')) {
					/** @noinspection PhpDeprecationInspection */	//BB added for phplint
    ...
    {
        if ($advanced) {
            //BB Not needed with auto-loading:		require_once 'extras/class.html2text.php';
            $htmlconverter = new CBhtml2text($html);
