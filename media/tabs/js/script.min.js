 ...
 */
(function($){$(document).ready(function(){if(typeof(window['nn_tabs_use_hash'])!="undefined"){nnTabs.init();}});nnTabs={init:function(){var $this=this;var nn_tabs_speed_fade=$('.nn_tabs .fade').length?$('.nn_tabs .fade').first().css('transition-duration'):'0.15s';var nn_tabs_speed_collapse=$('.nn_tabs .collapse').length?$('.nn_tabs .collapse').first().css('transition-duration'):'0.35s';this.insertCSSRule('.nn_tabs .fade','transition-duration: 1ms');this.insertCSSRule('.nn_tabs .collapse','transition-duration: 1ms');this.showByURL();this.showByHash();setTimeout((function(){$this.initActiveClasses();if(nn_tabs_use_hash){$this.initHashHandling();}
...
var $this=this;var $el=this.getElement(id);if(!$el.length){return;}$el.tab('show');$el.focus();},getElement:function(id){return this.getTabElement(id);},getTabElement:function(id){return $('a.nn_tabs-toggle[data-id="'+id+'"]');},getSliderElement:function(id){return $('#'+id+'.nn_sliders-body');},insertCSSRule:function(selector,rules){if(typeof document.styleSheets!='object'){return;}
if(!document.styleSheets.length){document.getElementsByTagName('head')[0].appendChild(document.createElement('style'));}
var sheet=document.styleSheets[document.styleSheets.length-1];if('cssText'in sheet){sheet.cssText=selector+'{'+rules+'}';return;}
if('insertRule'in sheet){try{var index=sheet.cssRules?sheet.cssRules.length:0;}catch(e){return;}
sheet.insertRule(selector+'{'+rules+'}',index);return;}
if('addRule'in sheet){sheet.addRule(selector,rules,index);return;}},showByURL:function(){var id=this.getUrlVar();if(id==''){return;}
